...
try
    ...
    %
    %
    stim_1 = load('C:\Users\Heinz Lab - NEL2\Desktop\Andrew_EEG\ACC_HeinzLab\Acquisition_MP\stims\stim_noise1_up1_1co.mat');
    stim_2 = load('C:\Users\Heinz Lab - NEL2\Desktop\Andrew_EEG\ACC_HeinzLab\Acquisition_MP\stims\stim_down1_up1_1co.mat');
    
    x_nu = stim_1.stim_noise5_up5_2co;
    x_du = stim_2.stim_down5_up5_2co;
...
%     triglist = randi(2,ntrials,1);
    ...
    for j = 1:ntrials
        ...
        stimTrigger = triglist(j); % 1 or 2
        %p_pol =  round(rand(1)); %1 for plus, 0 for minus
        
        if mod(stimTrigger,2)==0
            x = x_du;
            dur_p = length(x_du)/fs;
        else
            x = x_nu;
            dur_p = length(x_nu)/fs;
        end
